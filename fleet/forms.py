...
class DriverForm(ModelForm):
    ...
    driving_license = forms.CharField(widget=forms.TextInput(attrs=getAttrs('control','Permis de conduire')))
...
class VehicleForm(ModelForm):
    ...
    model = forms.CharField(widget=forms.TextInput(attrs=getAttrs('control','Modèle')))
    mark = forms.CharField(widget=forms.TextInput(attrs=getAttrs('control','Marque')))
    immatriculation = forms.CharField(widget=forms.TextInput(attrs=getAttrs('control','Immatriculation')))
    year = forms.IntegerField(widget=forms.NumberInput(attrs=getAttrs('control','Année')), validators=[MinValueValidator(1900)])
    fournisseur = forms.ModelChoiceField(queryset=Fournisseur.objects.filter(is_tracked=True), widget=forms.Select(attrs=getAttrs('select')))
